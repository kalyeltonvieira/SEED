...
auto mat4_perspective(double fov, double aspect, double z_near, double z_far) -> Mat4 {
    const auto f = 1.0 / std::tan(fov * 0.5);
...
auto color_clamp(Color c) -> Color {
    return color_new(std::max(0.0, std::min(1.0, c.r)), std::max(0.0, std::min(1.0, c.g)), std::max(0.0, std::min(1.0, c.b)), std::max(0.0, std::min(1.0, c.a)));
}
...
auto draw_triangle(Framebuffer& fb, Vec3 v0, Vec3 v1, Vec3 v2, Color c0, Color c1, Color c2) -> void {
    ...
    auto min_x = std::min(p0.x, std::min(p1.x, p2.x));
    auto max_x = std::max(p0.x, std::max(p1.x, p2.x));
    auto min_y = std::min(p0.y, std::min(p1.y, p2.y));
    auto max_y = std::max(p0.y, std::max(p1.y, p2.y));
...
auto mesh_sphere(double radius, int segments) -> Mesh {
    ...
    for (int lat = 0; lat < segments; lat++) {
        ...
        for (int lon = 0; lon < segments; lon++) {
            ...
            const auto p1 = vec3_new(radius * std::sin(theta1) * std::cos(phi1), radius * std::cos(theta1), radius * std::sin(theta1) * std::sin(phi1));
            const auto p2 = vec3_new(radius * std::sin(theta2) * std::cos(phi1), radius * std::cos(theta2), radius * std::sin(theta2) * std::sin(phi1));
            const auto p3 = vec3_new(radius * std::sin(theta2) * std::cos(phi2), radius * std::cos(theta2), radius * std::sin(theta2) * std::sin(phi2));
            const auto p4 = vec3_new(radius * std::sin(theta1) * std::cos(phi2), radius * std::cos(theta1), radius * std::sin(theta1) * std::sin(phi2));
...
auto phong_shade(Vec3 normal, Vec3 position, Vec3 camera_pos, Light light, Material material) -> Color {
    ...
    const auto diff = std::max(0.0, vec3_dot(normal, light_dir));
    ...
    const auto spec = std::pow(std::max(0.0, vec3_dot(view_dir, reflect_dir)), material.shininess);
...
auto framebuffer_to_terminal(const Framebuffer& fb, int width, int height) -> void {
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            const auto luminance = px_r * 0.299 + px_g * 0.587 + px_b * 0.114;
            ...
            const auto cr = px_r * 255.0;
            const auto cg = px_g * 255.0;
            const auto cb = px_b * 255.0;
            

            row_str = row_str + stdlib::terminal::fg_color(16 + (cr / 51.0)*36 + (cg / 51.0)*6 + (cb / 51.0)) + c;
        }
        stdlib::io::print(row_str + stdlib::terminal::reset() + "\n"s);
    }
}

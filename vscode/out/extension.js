...
// Global indexes
let modulesIndex = new Map();
let fileToModuleMap = new Map();
function getStdlibPath() {
    const compilerPath = vscode.workspace.getConfiguration("seed").get("compilerPath") || DEFAULT_COMPILER;
...
function parseFileImports(text) {
...
function getWordAndQualifier(document, position) {
            ...
            if (modName) {
                ...
                if (mod) {
                    ...
                    mod.symbols.forEach(sym => {
                        ...
                        else if (sym.kind === vscode.SymbolKind.Constant)
...
function findLocalDefinition(document, text, word) {
...
function functionCompletions(text) {
    const items = [];
    const seen = new Set();
...
function variableCompletions(text) {
    const items = [];
    const seen = new Set();
    const patterns = [
...
function pushVariable(items, seen, name, detail) {
    if (seen.has(name)) {
        return;
    }
    seen.add(name);
...
function getKeywordDocumentation(word) {
    const keywordDocs = {
...
async function runTests() {
    const testsPath = vscode.workspace.getConfiguration("seed").get("testsPath") || DEFAULT_TESTS;
...
async function runCompiler(filePath, mode, document) {
...
function buildCompilerArgs(filePath, mode) {
    if (mode === "run") {
        return ["run", filePath];
    }
    if (mode === "test") {
...
function resolveCompilerPath() {
    return vscode.workspace.getConfiguration("seed").get("compilerPath") || DEFAULT_COMPILER;
}
...
function parseCompilerDiagnostics(text, document) {
    const found = [];
    const lines = text.split(/\r?\n/);
    for (const line of lines) {
...
function makeDiagnostic(document, line, col, message, severity, code) {
    ...
    const diagnostic = new vscode.Diagnostic(new vscode.Range(safeLine, start, safeLine, end), message.trim(), severity);
...
function analyzeSeedDocument(document) {
    const text = document.getText();
    const result = [];
...
function hasNonExhaustiveMatch(text, typeName, variants) {
...
function hasUncheckedFallibleCall(text) {
...
function findLine(document, regex) {
...
function guessLineForCode(document, code) {
    const patterns = {
...
function countStructureChars(line, chars) {
    let count = 0;
    let inString = false;
...
function maybeShowWelcome(context) {
    const enabled = vscode.workspace.getConfiguration("seed").get("showWelcomeOnFirstSeedFile");
    if (!enabled || context.globalState.get("seed.welcomeShown")) {
        return;
    }
    context.globalState.update("seed.welcomeShown", true);
    showWelcome(context, false);
}
function showWelcome(context, explicit) {
...
function showDreamsPanel(context) {
...
function dreamsHtml(log, reviews) {

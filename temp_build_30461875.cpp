...
auto mat4_transform_direction(Mat4 m, Vec3 v) -> Vec3 {
    Vec3 {
...
auto color_red() -> Color {
auto color_green() -> Color {
auto color_blue() -> Color {
auto color_yellow() -> Color {
auto color_cyan() -> Color {
auto color_magenta() -> Color {
...
auto framebuffer_get_pixel(const Framebuffer& fb, int x, int y) -> Color {
    if (x >= 0 && x < fb.width && y >= 0 && y < fb.height) {
...
struct Scene {
    std::vector<(Mesh, Transform, Material)> meshes;
...
auto framebuffer_to_ascii(const Framebuffer& fb, int out_width, int out_height) -> std::string {
    const auto chars = " .:-=+*#%@"s;
    auto result = ""s;
...
auto framebuffer_to_terminal(const Framebuffer& fb, int out_width, int out_height) -> void {
    const auto chars = " .:-=+*#%@"s;
